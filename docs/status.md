...
---
## Project Summary
We are created an agent that learns to solve both simple and complex mazes. The agent's percepts are limited and is forced to gain its knowledge by iterating through multiple episodes of success and failure. At the end of each episode, the agent stores the reward feedback and uses this learned information in the next episode. The agent uses the State-Action-Reward-State-Action (Sarsa) algorithm to learn the Markov Decision Process (MDP) policy, which means the agent updates the policy based on the actions taken in the episode at learning rate alpha. After numerous episodes, the agent learns the sequence of actions that will return the highest reward which is equivalent to solving the maze. We also explored a part of maze generation by implementating the randomized Prim-Jarnik minimum spanning tree algorithm which we used to test our agent on the complex mazes it created. 

## Approach
Most of the code is in "sarsaattempt1.py." Our main algorithm is the Sarsa algorithm, which is described in Figure 6.9 on p. 146 of the book Introduction to Reinforcement Learning by Sutton and Barto. The MDP is simple and comparable to a gridworld. Each state is a position on the grid of the maze and is represented as a tuple of integers that are in the form (int+0.5, int+0.5) in the actual code. A state can transition to another state by moving one block on the grid in any of the four directions: north, west, east or south. A move in any of these directions are the only allowable actions in the predefined set of actions and each action is in the form "move____ 1" (i.e. strings, which are discrete malmo movement commands). This description of state and actions makes the agent "blind", similiar to tutorial_6.py. This means that the agent does not have any field of vision and does not know if there is a goal state. Although the knowledge available to the agent in the environment is limited, our agent was able to solve complicated mazes by learning what behavior would produce a higher reward after an episode ended, (i.e. when the agent reaches the goal or dies). We measured our agent's performance by the number of episodes it takes to "converge" (see the "Evaluation" section) and tested our agent on randomly generated mazes using the Prim-Jarnik MST algorithm, which is known to produce difficult mazes. 
