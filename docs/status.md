...
---
## Project Summary
We are created an agent that learns to solve both simple and complex mazes. The agent's percepts are limited and thus gains its knowledge through multiple episodes of success and failure by storing and learning upon the reward feedback for each episode. The agent uses this information to learn which sequence of actions will return the highest reward which is equivalent to solving the maze. 
